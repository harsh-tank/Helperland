...
export class DashboardService {
  ...
  public async rescheduleSR(date:Date, time:string, serviceId:number, userId:string): Promise<[number, ServiceRequest[]]> 
  {
    return this.dashboardRepository.rescheduleSR(date, time, serviceId, +userId);
  };
    ...
    .then(ser_Req => {
      ...
      if(ser_Req){
        for(let sr in ser_Req){
          ...
          if(Today_Date >= serviceRequestDate ){continue;}
  ...
  public async update_SR_Status(serviceId:number, userId:number): Promise<[number, ServiceRequest[]]> 
  {
    return this.dashboardRepository.update_SR_Status(serviceId,userId);
  };
  ...
  public Confirm_valid_date(date: string) {
    const updateDate = new Date(date.split("-").reverse(  ).join("-"));
