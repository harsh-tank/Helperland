    ...
    public getAllSRByUserId: RequestHandler = async (req,res): Promise<Response> => {
            ...
            .then((ser_Req) => {
              if (ser_Req) {
                if (ser_Req.length > 0) {
                  ...
                  return res
                    .status(404)
                    .json({ message: "All Service Request has been Completed" });
                }
              } else {
                return res
                  .status(404)
                  .json({ message: "User has not created any Service Request" });
              }
            })
            .catch((error: Error) => {
              console.log(error);
                ...
                if (serviceRequest) {
                  ...
                  if (serviceRequest.UserId === req.body.userId) {
                    if (serviceRequest.ServiceProviderId) {
                      ...
                      return this.dashboardService
                        .getAllSROfProvider(serviceRequest.ServiceProviderId)
      ...
      public Confirm_reschedule: RequestHandler = async (req,res): Promise<Response> => {
        ...
        if (req.params.serviceId) {
          return this.dashboardService.rescheduleSR(new Date(date),req.body.time,+req.params.serviceId)
            .then((ser_Request) => {
                            ...
                            return this.dashboardService.getProviderById(ser_Request.ServiceProviderId)
                              .then((Provider) => {
                                if (Provider?.Email) {
                                  ...
                                  return res.status(200).json({
                                      message:
                                        "Service Request is Cancelled Now",
                                    });
                                } else {
