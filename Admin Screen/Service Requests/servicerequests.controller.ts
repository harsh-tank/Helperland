...
export class ServiceRequestsController {
    ...
    public Return_refund: RequestHandler = async (req,res): Promise<Response> => {
    if (req.body.userTypeId === 2) {
      if(req.body.PaidAmount>req.body.RefundedAmount){
        return  this.serviceRequestsService.Return_refund(req.body.ServiceRequestId,req.body.RefundedAmount, req.body.userId)
        .then(ser_Req => {
          if(ser_Req){
            if(ser_Req[0] === 1){
              return res.status(422).json({ message: "Refund SuccessFull"});
            }else{
              return res.status(422).json({ message: "Not Refunded"});
            }
          }else{
            return res.status(404).json({ message: "No SR Found to Refund"});
          }
        })
        .catch((error: Error) => {
          console.log(error);
        ...
        return res.status(401).json({ message: "Refund Amount Must be Less Than SR Charges"});
      }
    } else {
      return res.status(401).json({ message: "unauthorised User" });
    }
