    ...
    public constructor(private readonly serviceRequestsRepository: ServiceRequestsRepository) { 
        ...
        public async getAllSR(): Promise<Print_Req_Data[] | null> {
          ...
          public async filter_feature_SR(requests:Print_Req_Data[], filters:filter_features){
            ...
            if(filters.UserId){
              if(filter_Info){
                filter_Info = filter_Info.filter(x => {return x.CustomerDetails.UserId === filters.UserId
                });
              }else{
                filter_Info = requests.filter(x => {return x.CustomerDetails.UserId === filters.UserId
                });
              }
            }
            if(filters.ServiceProviderId){
              if(filter_Info){
                filter_Info = filter_Info.filter(x => {return x.ServiceProvider.ServiceProviderId === filters.ServiceProviderId
                });
              }else{
                filter_Info = requests.filter(x => {return x.ServiceProvider.ServiceProviderId === filters.ServiceProviderId
                });
              }
            }
